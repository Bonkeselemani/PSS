...
Namespace Gui.TracFone
    Public Class frmTFOOBA
        ...
        Private _SelectedBox_Loc_ID As Integer = 0
...
#End Region
        ...
        Private Sub frmTFOOBA_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Try
                ...
                If Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID Then
                    'do nothing here. Load failcodes when scan box
                Else
                    Me.LoadFailureCodes()
                End If
        ...
        '*****************************************************************
        Private Sub LoadFailureCodes(Optional ByVal iLoc_ID As Integer = 0)
            ...
            Try
                If Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID Then
                    Dim iVivint_Loc_ID As Integer = iLoc_ID
                    Dim iVivint_MCode_ID As Integer = 0
                    Dim iProd_ID As Integer = PSS.Data.Buisness.VV.Vivint.Vivint_Product_ID
                    Dim strLoc As String = ""
                    Dim dtFC As DataTable

                    Me.cboCodes.ClearItems()

                    strLoc = Me._objPreTest.getCustomerLocation(Me._iMenuCustID, iVivint_Loc_ID)
                    ...
                    If iVivint_MCode_ID > 0 Then
                        dtFC = Me._objPreTest.GetWiKo_FailedCodes(iProd_ID, iVivint_MCode_ID)
                        Dim ic As Integer = dtFC.Rows.Count
                        If Not IsNothing(dtFC) Then
                            Misc.PopulateC1DropDownList(Me.cboCodes, dtFC, "DCode_LDesc", "DCode_ID")
        ...
        Private Sub txtSN_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtSN.KeyUp
            Dim dt1, dtLastQCRecord, dtNTF As DataTable
            ...
            Try
                If e.KeyValue = 13 Then
                    ...
                    ''******************************************
                    If Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID Then
                        dt1 = Generic.GetDeviceInfoInWIP(Trim(Me.txtSN.Text), Me._iMenuCustID, Me._SelectedBox_Loc_ID, True)
                    Else
                    ...
                    If dt1.Rows.Count > 0 Then
                        If Me._iMenuCustID = PSS.Data.Buisness.TracFone.BuildShipPallet.TracFone_CUSTOMER_ID AndAlso (dt1.Rows(0)("WorkStation").ToString.Trim.ToUpper <> "BOX" And dt1.Rows(0)("WorkStation").ToString.Trim.ToUpper <> "AQL-OBA") Then
                            If dt1.Rows(0)("WorkStation").ToString.Trim.ToUpper <> Me._strScreenName.Trim.ToUpper Then
                                ...
                                Me.txtSN.Text = "" : Exit Sub
                            End If
                        ElseIf Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID AndAlso dt1.Rows(0)("WorkStation").ToString.Trim.ToUpper <> "PRODUCE" Then
                            MessageBox.Show("The device belongs to " & dt1.Rows(0)("WorkStation").ToString & ". Workstation must be 'PRODUCE'.", Me._strScreenName, MessageBoxButtons.OK, MessageBoxIcon.Stop, MessageBoxDefaultButton.Button1)
                            Me.txtSN.Text = "" : Exit Sub
                        End If
                        ...
                        'Per QC team request, enable multiple AQL-OBA tests for a device
                        ...
                        'Check for FQA 
                        '******************************************
                        ' for Vinsmart special Project only 
                        'SKIPP FQA FOR V350U Rexford 08202021 Reflash Rework ASN.xlsx DEVICES 
                        Dim strFilename As String
                        Dim dtReworkFilename As DataTable = Me._objVinsmart_SP.GetSourceFile(Me.txtSN.Text.Trim, _iMenuCustID)
                        If dtReworkFilename.Rows.Count > 0 Then
                            strFilename = dtReworkFilename.Rows(0).Item("SourceFile")
                        Else
                            strFilename = ""
                        End If
                        If Me._iMenuCustID = PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_CUSTOMER_ID AndAlso Not strFilename = Me._objVinsmart.strDefSourceFile Then
                        ...
                        If Me._iMenuCustID = PSS.Data.Buisness.TracFone.BuildShipPallet.TracFone_CUSTOMER_ID Then
                            ...
                            dtNTF = objTFMisc.GetNTFDeviceID(dt1.Rows(0)("Device_id"), Me._iMenuCustID)
                            If dtNTF.Rows.Count > 0 Then Me._bIsNTF = True
                        End If

                        '******************************************
                        'Validate billdate
                        '******************************************
                        'added by amazech-Thanga 28.07.2021
                        If dt1.Rows(0)("Loc_id") <> PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_Special_LOC_ID Then
                            If Me._bIsNTF = False AndAlso objQC.HasBillDate(dt1.Rows(0)("Device_id")) = False Then
                        ...
                        If Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID Then
                            ...
                            Dim iManufWrty As Integer = Convert.ToInt32(dt1.Rows(0)("Device_ManufWrty"))
                            Dim dtWrty As DataTable, row As DataRow
                            Me.lblDevRepType.Visible = False : Me.lblDateCode.Visible = False
                            dtWrty = objVivint.getWarrantyTypeData 'Wrty_ID, Wrty_Desc, BoxNamePart
                            Me.lblWrtyStatus.Visible = True
                            For Each row In dtWrty.Rows
                                If iManufWrty = Convert.ToInt32(row("Wrty_ID")) Then
                                    Me.lblWrtyStatus.Text = row("Wrty_Desc")
                                End If
                            Next
                            objVivint = Nothing
                        Else
                            Me.lblDevRepType.Visible = True
                            If dt1.Rows(0)("FuncRep") = 1 Then
                                Me.lblDevRepType.Text = "Functional"
                            ElseIf dt1.Rows(0)("FuncRep") = 0 Then
                                Me.lblDevRepType.Text = "Cosmetic"
                            ElseIf dt1.Rows(0)("FuncRep") = 3 Then
            ...
            Try
                ...
                ElseIf dt.Select("Pallet_Invalid = 0").Length = 1 Then
                    ...
                    Else
                        ...
                        '********************************
                        'Load FailureCodes for Vivint
                        '********************************
                        If Me._iMenuCustID = PSS.Data.Buisness.VV.Vivint.Vivint_CUSTOMER_ID Then
                            Me._SelectedBox_Loc_ID = R1("Loc_ID")
                            LoadFailureCodes(Me._SelectedBox_Loc_ID)
                        End If
            ...
            Try
                ...
                Else
                    ...
                    ElseIf dt.Rows(0)("WorkStation").ToString.Trim.Length = 0 Then
                        ...
                        'if the selected customer is Vinsmart, skip this check
                    ElseIf _iMenuCustID <> Data.Buisness.Vinsmart.Vinsmart.Vinsmart_CUSTOMER_ID AndAlso Misc.ValidateFrStationOfScreenInWorkFlow(Me._strScreenName, dt.Rows(0)("WorkStation").ToString.Trim, PSS.Data.Buisness.TracFone.BuildShipPallet.TracFone_CUSTOMER_ID, 0) = False Then
                        Exit Sub
                    ...
                    Else
                        ...
                        Else
                            ...
                            i = objTFMisc.PushShipBoxToNextStation(Me._iPalletID, strNextStation, Core.ApplicationUser.IDuser, Me._strScreenName, Me.Name)
                            If i > 0 Then
        ...
        Private Sub lblBoxPassQty_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lblBoxPassQty.Click

        End Sub

        Private Sub lblBoxPassQty_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblBoxPassQty.TextChanged
