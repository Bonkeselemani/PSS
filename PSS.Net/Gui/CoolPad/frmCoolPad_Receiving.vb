...
Namespace Gui.CP
    Public Class frmCoolPad_Receiving
...
#Region " Windows Form Designer generated code "
        ...
        <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
            Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmCoolPad_Receiving))
            ...
            Me.Name = "frmCoolPad_Receiving"
            Me.Text = "frmCoolPad_Receiving"
...
#End Region
        ...
        Private Sub frmCoolPad_Receiving_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            Dim strCustLoc As String = ""
            Dim dtLoc, dtLoc_SeedStock, dtLoc_EndUser As DataTable
            Dim iLoc_ID As Integer = 0 'PSS.Data.Buisness.CP.CoolPad.CoolPad_CP1_Loc_ID
            ...
            Try
                ...
                Me._RecvDT = Me._objCoolPad_Receiving.GetRecvTableDef
                Me._SeedStockRecvDT = Me._objCoolPad_Receiving.GetSeedStockRecvTableDef
                Me._EndUserRecvDT = Me._objCoolPad_Receiving.GetEndUserRecvTableDef

                'Location
                dtLoc = Me._objCoolPad_Receiving.GetCoolPadLocations(Me._iCust_ID, True)
                ...
                '    dtModel = Me._objCoolPad_Receiving.getModelData(PSS.Data.Buisness.CP.CoolPad.CoolPad_Product_ID, Me._iCust_ID, Me.cboLocation.SelectedValue)

                '    Misc.PopulateC1DropDownList(Me.cboASN_In_Sku, dtModel, "SN_IN_SKU", "Model_ID")
                '    Me.cboASN_In_Sku.SelectedValue = 0
                'Else
                '    MessageBox.Show("Please select a location.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Stop, MessageBoxDefaultButton.Button1)
                'End If
        ...
        Private Sub cboLocation_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboLocation.SelectedValueChanged
            ...
            Try
                ...
                If Me.cboLocation.SelectedValue > 0 Then
                    ...
                    Me.cboASN_In_Sku.Text = "--Select--"
                    lblPSSModel.Text = String.Empty
        ...
        Private Sub ProcessSN()
            ...
            Dim row As DataRow
            Dim iMaxQty As Integer = Me._objCoolPad.getMaxReceivingBoxQty
            ...
            Try
                ...
                ElseIf Not Me.lblPSSModel.Text = Me.cboASN_In_Sku.DataSource.Table.Select("Model_ID = " & Me.cboASN_In_Sku.SelectedValue)(0)("Model_Desc") Then
                    ...
                    ClearSnTextBox()
                Else
                    dt = Me._objCoolPad_Receiving.getReceivingData(Me._iCust_ID, Me._iLoc_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeBulk_ID, strSN)
                    ...
                    Else '=1
                        ...
                        ElseIf Not PSS.Data.Buisness.CP.CoolPad.CoolPad_RepairProgramType.Contains(strRepairProgramType.Trim.ToUpper) Then 'RepairProgramType
                        ...
                        Else
                            ...
                            iTray_ID = Me._objCoolPad_Receiving.getTayID(Me._iUserID, Me._strUser, iWO_ID, strTrayMemo)
                            ...
                            Else 'Ready to receive 
                                ...
                                If Me._RecvDT.Rows.Count = 0 Then 'first device
                                    ...
                                    Me._strRecvBoxName = Me._objCoolPad_Receiving.CreateWarehouseBoxName(iModel_ID, iWrtyFlag, Me._iWB_ID)
                                ...
                                Else 'validate with recev data
                                    ...
                                    If strValidatedMsg.Length > 0 Then
                                        ...
                                        Me.txtSN.Text = String.Empty : Me.txtSN.Focus()
                                        Exit Sub
                                    End If
                                End If

                                'receiving now------------------------
                                bReceived = Me._objCoolPad_Receiving.ReceiveDataIntoSystem(Me._iLoc_ID, iWO_ID, iModel_ID, strSN, strManufDate, strDateTime, _
                                ...
                                If bReceived Then
                                    ...
                                    Me.txtSN.Text = "" : Me.txtSN.Text = String.Empty : Me.txtSN.Focus()
                                Else
        ...
        Private Function IsReceivedUnshipped(ByVal iLoc_ID As Integer, ByVal strSN As String) As Boolean
            ...
            Try
                dt = Me._objCoolPad_Receiving.getReceivedUnshipped(iLoc_ID, strSN)
                If dt.Rows.Count > 0 Then
                    Return True
                Else
                    Return False
        ...
        Private Function GoValidation(ByVal RecvDT As DataTable, ByVal strSN As String, ByVal strSku As String, _
            ...
            Try
                ...
                If _iLoc_ID <> _objCoolPad.CoolPad_Special_LOC_ID Then
        ...
        Private Sub btnCloseBox_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCloseBox.Click
            ...
            Dim iASN_OrderType_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeBulk_ID
            ...
            Try
                ...
                ElseIf MsgBox("Do you want to close the box?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    'Close box
                    i = Me._objCoolPad_Receiving.ColseWarehouseBox(Me._iWB_ID, Me._RecvDT.Rows.Count, strBoxStage)

                    If i > 0 Then
                        'print Label
                        Me._objCoolPad_Receiving.PrintReceivedBoxLabel(iASN_OrderType_ID, Me._strRecvBoxName, Me._RecvDT.Rows.Count, Me._RecvDT.Rows(0).Item("SKU"), Me._RecvDT.Rows(0).Item("PSS_Model"), _
        ...
        Private Sub cboSeedstockLocation_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboSeedStockLocation.SelectedValueChanged
            ...
            Try
                ...
                If Me.cboSeedStockLocation.SelectedValue > 0 Then
                    dtModel = Me._objCoolPad_Receiving.getModelData(PSS.Data.Buisness.CP.CoolPad.CoolPad_Product_ID, Me._iCust_ID, Me.cboSeedStockLocation.SelectedValue)
        ...
        Private Sub ProcessSeedStockSN()
            ...
            Dim strWorkDate As String = Generic.GetWorkDate(iShift_ID)
            Dim iTray_ID As Integer = 0
            Dim strTrayMemo As String = "CoolPad SeedStock Receiving"
            ...
            Try
                ...
                ElseIf Not Me.lblSeedStockPSSModel.Text = Me.cboSeedStockASN_In_Sku.DataSource.Table.Select("Model_ID = " & Me.cboSeedStockASN_In_Sku.SelectedValue)(0)("Model_Desc") Then
                    ...
                    ClearSnTextBox()
                Else
                    dt = Me._objCoolPad_Receiving.getSeedStockReceivingData(Me._iCust_ID, Me._iSeedStockLoc_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeSeedStock_ID, strSN)
                        ...
                        Else
                            ...
                            Else 'Ready to receive 
                                ...
                                If Me._SeedStockRecvDT.Rows.Count = 0 Then 'first device
                                    ...
                                    Me._strSeedStockRecvBoxName = Me._objCoolPad_Receiving.CreateWarehouseBoxName(iModel_ID, iWrtyFlag, Me._iSeedStockWB_ID)
                                ...
                                'receiving now------------------------
                                bReceived = Me._objCoolPad_Receiving.ReceiveDataIntoSystem(Me._iSeedStockLoc_ID, iWO_ID, iModel_ID, strSN, strManufDate, strDateTime, _
        ...
        Private Sub btnSeedStockCloseBox_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSeedStockCloseBox.Click
            ...
            Dim iASN_OrderType_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeSeedStock_ID
            ...
            Try
                ...
                ElseIf MsgBox("Do you want to close the box?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    'Close box
                    i = Me._objCoolPad_Receiving.ColseWarehouseBox(Me._iSeedStockWB_ID, Me._SeedStockRecvDT.Rows.Count, strBoxStage)

                    If i > 0 Then
                        'print Label
                        Me._objCoolPad_Receiving.PrintReceivedBoxLabel(iASN_OrderType_ID, Me._strSeedStockRecvBoxName, Me._SeedStockRecvDT.Rows.Count, Me._SeedStockRecvDT.Rows(0).Item("SKU"), Me._SeedStockRecvDT.Rows(0).Item("PSS_Model"), _
        ...
        Private Sub btnReprintBoxLabel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReprintBoxLabel.Click
            ...
            Dim iLoc_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_CP1_Loc_ID
            ...
            Try
                If Me.rbtByBoxName.Checked Then 'BY BOX NAME -1-------------------------------------------------------------------------------------------------------------
                    ...
                    dt = Me._objCoolPad_Receiving.getReceivedBulkBoxData(Me._iCust_ID, iLoc_ID, strInput, 1)
                    If dt.Rows.Count > 0 Then
                        iBoxQty = Convert.ToInt32(dt.Rows(0).Item("Recv_Qty"))
                        If iBoxQty = dt.Rows.Count Then
                            'print Label
                            Me._objCoolPad_Receiving.PrintReceivedBoxLabel(Convert.ToInt32(dt.Rows(0).Item("BulkOrderType_ID")), dt.Rows(0).Item("BoxName"), _
                ...
                ElseIf Me.rbtBySN.Checked Then 'BY SN (IMEI) -2------------------------------------------------------------------------------------------------------------
                    ...
                    dtSN = Me._objCoolPad_Receiving.getReceivedBulkBoxData(Me._iCust_ID, iLoc_ID, strInput, 2) 'get data for the SN
                    If dtSN.Rows.Count > 0 Then
                        ...
                        If arrLstBoxes.Count = 1 Then 'one box
                            dt = Me._objCoolPad_Receiving.getReceivedBulkBoxData(Me._iCust_ID, iLoc_ID, arrLstBoxes(0), 1) 'get box data  
                            iBoxQty = Convert.ToInt32(dt.Rows(0).Item("Recv_Qty"))
                            If iBoxQty = dt.Rows.Count Then
                                'print Label
                                Me._objCoolPad_Receiving.PrintReceivedBoxLabel(Convert.ToInt32(dt.Rows(0).Item("BulkOrderType_ID")), dt.Rows(0).Item("BoxName"), _
                        ...
                        ElseIf arrLstBoxes.Count > 1 Then 'more boxes
                            ...
                            dt = Me._objCoolPad_Receiving.getReceivedBulkBoxData(Me._iCust_ID, iLoc_ID, selectedBoxName, 1)  'get box data
                            iBoxQty = Convert.ToInt32(dt.Rows(0).Item("Recv_Qty"))
                            If iBoxQty = dt.Rows.Count Then
                                'print Label
                                Me._objCoolPad_Receiving.PrintReceivedBoxLabel(Convert.ToInt32(dt.Rows(0).Item("BulkOrderType_ID")), dt.Rows(0).Item("BoxName"), _
                ...
                ElseIf Me.rbtByPoNumber.Checked Then 'BY PO Number -3------------------------------------------------------------------------------------------------------
                    ...
                    dtPO = Me._objCoolPad_Receiving.getReceivedBulkBoxData(Me._iCust_ID, iLoc_ID, strInput, 3)
        ...
        Private Sub cboEndUserLocation_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboEndUserLocation.SelectedValueChanged
            ...
            Try
                ...
                If Me.cboEndUserLocation.SelectedValue > 0 Then
                    dtModel = Me._objCoolPad_Receiving.getModelData(PSS.Data.Buisness.CP.CoolPad.CoolPad_Product_ID, Me._iCust_ID, Me.cboEndUserLocation.SelectedValue)
        ...
        Private Sub ProcessEndUserSN()
            ...
            Dim strWorkDate As String = Generic.GetWorkDate(iShift_ID)
            Dim iTray_ID As Integer = 0
            Dim strTrayMemo As String = "CoolPad EndUser Receiving"
            ...
            Try
                ...
                Else
                    If Me.chkBoxByRMA.Checked Then
                        dt = Me._objCoolPad_Receiving.getEndUserReceivingData(Me._iCust_ID, Me._iEndUserLoc_ID, _
                                                                              PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeEndUser_ID, strSN, 2)
                    Else
                        dt = Me._objCoolPad_Receiving.getEndUserReceivingData(Me._iCust_ID, Me._iEndUserLoc_ID, _
                                                                              PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeEndUser_ID, strSN, 1)
                    End If
                        ...
                        Else
                            ...
                            Else 'Ready to receive 
                                ...
                                If Me._EndUserRecvDT.Rows.Count = 0 Then 'first device
                                    ...
                                    Me._strEndUserRecvBoxName = Me._objCoolPad_Receiving.CreateWarehouseBoxName(iModel_ID, iWrtyFlag, Me._iEndUserWB_ID)
                                ...
                                'receiving now------------------------
                                bReceived = Me._objCoolPad_Receiving.ReceiveDataIntoSystem(Me._iEndUserLoc_ID, iWO_ID, iModel_ID, strSN, strManufDate, strDateTime, _
        ...
        Private Sub ProcessEndUserSN_AllowReceivingMixedSKUsInBox()
            ...
            Try
                ...
                ElseIf Me.IsReceivedUnshipped(Me._iEndUserLoc_ID, strSN) Then
                    ...
                    '    MessageBox.Show("Mismatched PSS Model.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Else
                    ' dt = Me._objCoolPad_Receiving.getEndUserReceivingData(Me._iCust_ID, Me._iEndUserLoc_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeEndUser_ID, strSN)
                    ...
                    Else '=1
                        ...
                        Else
                            ...
                            Else 'Ready to receive 
                                ...
                                If Me._EndUserRecvDT.Rows.Count = 0 Then 'first device
                                    Me._iEndUserWB_ID = 0 : Me._iEndUserRecID = 0
                                    Me._strEndUserRecvBoxName = Me._objCoolPad_Receiving.CreateWarehouseBoxName(0, iWrtyFlag, Me._iEndUserWB_ID) 'allowmixed model in a box, so use 0  for iModel_ID
                                    Me.lblEndUserBoxName.Text = Me._strEndUserRecvBoxName
                                    Me.txtEndUserMaxBoxQty.Text = iMaxQty.ToString
                                Else 'validate with recev data
                                    Dim strValidatedMsg As String = Me.GoEndUserValidation(Me._EndUserRecvDT, strSN, iMaxQty).Trim
        ...
        'Private Function IsValidRMAShipDate(ByVal dt As DataTable) As Boolean
        '    Dim row As DataRow
        '    Dim bRet As Boolean = False
        '    Try
        '        For Each row In dt.Rows
        '            If row.IsNull("RMA_Ship_Date_Text") OrElse Convert.ToString(row("RMA_Ship_Date_Text")).Trim.Length = 0 Then
        '                bRet = False
        '            ElseIf IsDate(Convert.ToString(row("RMA_Ship_Date_Text")).Trim) Then
        '                bRet = True
        '            Else
        '                bRet = False
        '            End If
        ...
        Private Sub btnEndUserCloseBox_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEndUserCloseBox.Click
            ...
            Dim iASN_OrderType_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeEndUser_ID
            ...
            Try
                ...
                ElseIf MsgBox("Do you want to close the box?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    'Close box
                    i = Me._objCoolPad_Receiving.ColseWarehouseBox(Me._iEndUserWB_ID, Me._EndUserRecvDT.Rows.Count, strBoxStage)

                    If i > 0 Then
                        'print Label
                        Me._objCoolPad_Receiving.PrintReceivedBoxLabel(iASN_OrderType_ID, Me._strEndUserRecvBoxName, Me._EndUserRecvDT.Rows.Count, Me._EndUserRecvDT.Rows(0).Item("SKU"), Me._EndUserRecvDT.Rows(0).Item("PSS_Model"), _
            ...
            Try
                ...
                ElseIf MsgBox("Do you want to close the box?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                    ...
                    If i > 0 Then
                        ...
                        End If
                        'print Label
                        Me._objCoolPad_Receiving.PrintReceivedBoxLabel(iASN_OrderType_ID, Me._strEndUserRecvBoxName, Me._EndUserRecvDT.Rows.Count, strMultipleSKUs, strMultipleModels, _
                                                "", "", "", strWHLocation, strOSD, Me._EndUserRecvDT.Rows(0).Item("Customer"), strASN_Category)
                    Else
                        MessageBox.Show("Failed to Close the box. See IT.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                    End If
        ...
        Private Sub btnReprintEndUserBoxLabel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReprintEndUserBoxLabel.Click
            ...
            Try
                If Me.rbtByBoxName.Checked Then 'BY BOX NAME -1-------------------------------------------------------------------------------------------------------------
                    strInput = InputBox("Enter a box name:", "Box Name").Trim
                    If strInput = "" Then Throw New Exception("Please enter a valid box name.")
                    dt = Me._objCoolPad_Receiving.getReceivedEndUserBoxData(Me._iCust_ID, iLoc_ID, strInput, 1)
                    If dt.Rows.Count > 0 Then
                        iBoxQty = Convert.ToInt32(dt.Rows(0).Item("Recv_Qty"))
                        If iBoxQty = dt.Rows.Count Then
                            'print Label
                ...
                ElseIf Me.rbtBySN.Checked Then 'BY SN (IMEI) -2------------------------------------------------------------------------------------------------------------
                    strInput = InputBox("Enter a SN (IMEI):", "SN (IMEI)").Trim
                    If strInput = "" Then Throw New Exception("Please enter a valid SN (IMEI).")
                    dtSN = Me._objCoolPad_Receiving.getReceivedEndUserBoxData(Me._iCust_ID, iLoc_ID, strInput, 2) 'get data for the SN
                    If dtSN.Rows.Count > 0 Then
                        For Each row In dtSN.Rows
                            If Not arrLstBoxes.Contains(row("BoxName")) Then
                                arrLstBoxes.Add(row("BoxName")) : arrlstRecvDates.Add(row("Device_DateRec"))
                        ...
                        Next
                        If arrLstBoxes.Count = 1 Then 'one box
                            dt = Me._objCoolPad_Receiving.getReceivedEndUserBoxData(Me._iCust_ID, iLoc_ID, arrLstBoxes(0), 1) 'get box data  
                            iBoxQty = Convert.ToInt32(dt.Rows(0).Item("Recv_Qty"))
                            If iBoxQty = dt.Rows.Count Then
                                'print Label
                                Me._objCoolPad_Receiving.PrintReceivedBoxLabel(Convert.ToInt32(dt.Rows(0).Item("BulkOrderType_ID")), dt.Rows(0).Item("BoxName"), _
