...
Namespace Gui.CP
    Public Class frmCoolPad_Swap
        ...
        Private _objCoolPad_Swap As PSS.Data.Buisness.CP.CoolPad_Swap
...
#Region " Windows Form Designer generated code "
        ...
        Public Sub New(ByVal strScreenName As String, ByVal iCust_ID As Integer)
            ...
            Me._objCoolPad_Swap = New PSS.Data.Buisness.CP.CoolPad_Swap()
        ...
        <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
            Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(frmCoolPad_Swap))
            ...
            Me.tdgData1.Location = New System.Drawing.Point(24, 152)
            ...
            Me.btnUndoOneSN.Location = New System.Drawing.Point(544, 72)
            Me.btnUndoOneSN.Name = "btnUndoOneSN"
            Me.btnUndoOneSN.Size = New System.Drawing.Size(200, 32)
            Me.btnUndoOneSN.TabIndex = 143
            ...
            Me.btnUnDoAllSNs.Location = New System.Drawing.Point(544, 112)
            Me.btnUnDoAllSNs.Name = "btnUnDoAllSNs"
            Me.btnUnDoAllSNs.Size = New System.Drawing.Size(200, 32)
            Me.btnUnDoAllSNs.TabIndex = 144
            ...
            Me.lblLocation.Location = New System.Drawing.Point(48, 48)
            ...
            Me.cboCustomer.Location = New System.Drawing.Point(128, 16)
            ...
            Me.Name = "frmCoolPad_Swap"
            Me.Text = "frmCoolPad_Swap"
...
#End Region
        ...
        Private Sub frmCoolPad_Swap_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Private Sub btnSwap_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
            ...
            '    dbgSource.DataSource = _objCoolPad_Swap.updatedbg(lSeedstock, SN)
            'End If
        End Sub
        ...
        Private Sub txtSwappedSN_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtSwappedSN.KeyUp
            Try
                If e.KeyCode = Keys.Enter Then
                    If Me.txtSwappedSN.Text.Trim.Length > 0 Then
                        Me.ProcessSN(PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeBulk_ID)
                    ElseIf Me.txtSwappedSN.Text.Trim.Length > 0 AndAlso Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID Then
                        Me.ProcessSN(PSS.Data.Buisness.WIKO.WIKO.WIKO_OrderTypeBulk_ID)
                    End If
                End If
            Catch ex As Exception
        ...
        Private Sub txtSeedStockSN_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtSeedStockSN.KeyUp
            Try
                If e.KeyCode = Keys.Enter Then
                    If Me.txtSwappedSN.Text.Trim.Length > 0 AndAlso Me._iCust_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_CUSTOMER_ID Then
                        Me.ProcessSN(PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeSeedStock_ID)
                    ElseIf Me.txtSwappedSN.Text.Trim.Length > 0 AndAlso Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID Then
                        Me.ProcessSN(PSS.Data.Buisness.WIKO.WIKO.WIKO_OrderTypeSeedStock_ID)
                    End If
                End If
            Catch ex As Exception
        ...
        '    'Dim strTrayMemo As String = "CoolPad Receiving"

        '    'Swap: only BER bulk SN can be swapped by a seedstock SN. This seedstock SN must be billed as swapped_BillCode_ID
        '    'Reivsed all 02-08-2021

        '    Try
        '        If iSN_Type = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeBulk_ID Then
        '            Me.txtSeedStockSN.Enabled = False
        ...
        '            Me._dtBulk = Me._objCoolPad_Swap.getDeviceData(Me._iCust_ID, Me.cboLocation.SelectedValue, strSN, iSN_Type, "")

        '            If Not Me._dtBulk.Rows.Count > 0 Then
        '                MessageBox.Show("Can't find this bulk SN '" & strSN & "'.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        '                Me.txtSwappedSN.SelectAll() : Me.txtSwappedSN.Focus() : Exit Sub
        '            ElseIf Me._dtBulk.Rows.Count > 1 Then
        '                MessageBox.Show("Duplicate bulk SNs.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        '                Me.txtSwappedSN.SelectAll() : Me.txtSwappedSN.Focus() : Exit Sub
        '            Else '=1
        ...
        '                dtBill = Me._objCoolPad_Swap.getDeviceBillData(iBulk_Device_ID)
        ...
        '                    Me.txtSwappedSN.SelectAll() : Me.txtSwappedSN.Focus() : Exit Sub
        '                ElseIf dtBill.Rows.Count > 1 AndAlso Me._objCoolPad_Swap.Is_BER_Device(iBulk_Device_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_BER_BillCode_ID) Then
        '                    MessageBox.Show("The device has BER, but multiple billcodes.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        '                    Me.txtSwappedSN.SelectAll() : Me.txtSwappedSN.Focus() : Exit Sub
        '                ElseIf dtBill.Rows.Count >= 1 AndAlso Not Me._objCoolPad_Swap.Is_BER_Device(iBulk_Device_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_BER_BillCode_ID) Then
        '                    MessageBox.Show("The device has a BER device (can't be swapped).", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        '                    Me.txtSwappedSN.SelectAll() : Me.txtSwappedSN.Focus() : Exit Sub
        '                ElseIf dtBill.Rows.Count = 1 AndAlso Me._objCoolPad_Swap.Is_BER_Device(iBulk_Device_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_BER_BillCode_ID) Then
        ...
        '                End If
        '            End If
        '        ElseIf iSN_Type = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeSeedStock_ID Then
        '            strSN = Me.txtSeedStockSN.Text.Trim
        '            Dim strWiKoCricketOrATT As String = ""


        '            dtSeedStock = Me._objCoolPad_Swap.getDeviceData(Me._iCust_ID, Me.cboLocation.SelectedValue, strSN, iSN_Type, strWiKoCricketOrATT)
        ...
        '                If Me._objCoolPad_Swap.Has_Swap_Bill_Code(iSeeStock_Device_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_BillCode_ID) Then
        ...
        '                    Me.txtSeedStockSN.SelectAll() : Me.txtSeedStockSN.Focus() : Exit Sub
        '                End If
        '                If Not Me._iModel_ID = Convert.ToInt32(Me._dtBulk.Rows(0).Item("Model_ID")) Then
        ...
        '                    Dim iLaborLevel As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_LabelLevel
        '                    Dim iPrcGroup_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_PrcGroup_ID
        '                    Dim iPsPRice_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_PSPrice_ID
        '                    Dim iSwap_BillCode_ID As Integer = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_BillCode_ID
        '                    Dim strWorkStation As String = PSS.Data.Buisness.CP.CoolPad.CoolPad_DeviceSwap_WorkStation
        ...
        '                    If MsgBox("Do you want to swap it?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
        '                        'get labor charge
        '                        dtTmp = Me._objCoolPad_Swap.getLaborChargeData(iLaborLevel, iPrcGroup_ID)
        ...
        '                        End If
        '                        'Get part number
        '                        dtTmp = Me._objCoolPad_Swap.getPartNumberData(iPsPRice_ID)
        '                        If Not dtTmp.Rows.Count > 0 Then
        ...
        '                        'Update
        '                        k = Me._objCoolPad_Swap.UpdateSwappedData(iSeeStock_Device_ID, iBulk_Device_ID, iSwap_BillCode_ID, strPartNum, Me._iUserID, strDateTime, _
        ...
        Private Sub ProcessSN(ByVal iSN_Type As Integer)
            ...
            Try
                ...
                ElseIf iSN_Type = PSS.Data.Buisness.CP.CoolPad.CoolPad_OrderTypeSeedStock_ID Then
                    ...
                    If Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID Then
                        iLoc_ID = Convert.ToInt32(Me._dtBulk.Rows(0).Item("Loc_ID"))
                        If iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCricket_LOC_ID Then
                            strWiKoCricketOrATT = PSS.Data.Buisness.WIKO.WIKO.WIKO_SeedStockSourceType_Cricket.Trim
                        ElseIf iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_LOC_ID OrElse iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttFedEx_LOC_ID Then
                            strWiKoCricketOrATT = PSS.Data.Buisness.WIKO.WIKO.WIKO_SeedStockSourceType_ATT.Trim
                        Else
                            MessageBox.Show("Can't determine WIKO Location. See IT.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                            Me.txtSeedStockSN.SelectAll() : Me.txtSeedStockSN.Focus() : Exit Sub
                        End If
                    End If
                    ...
                    Else '=1
                        ...
                        If Me._objCoolPad_Swap.Has_Swap_Bill_Code(iSeeStock_Device_ID, PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_BillCode_ID) Then
                            ...
                            Me.txtSeedStockSN.SelectAll() : Me.txtSeedStockSN.Focus() : Exit Sub
                        End If
                        'dtBill = Me._objCoolPad_Swap.getDeviceBillData(iSeeStock_Device_ID)
                        'If dtBill.Rows.Count > 0 Then
                        '    MessageBox.Show("This seedstock device has bill code(s). See IT.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        '    Me.txtSeedStockSN.SelectAll() : Me.txtSeedStockSN.Focus() : Exit Sub
                        'End If

                        If Me._iCust_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_CUSTOMER_ID _
                        ...
                        If Me._dtBulk.Rows.Count = 1 AndAlso dtSeedStock.Rows.Count = 1 Then
                            Dim row As DataRow, row2 As DataRow
                            Dim i As Integer = 0, j As Integer = 0, k As Integer = 0
                            Dim iLaborLevel As Integer = 0 ' PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_LabelLevel
                            Dim iPrcGroup_ID As Integer = 0 'PSS.Data.Buisness.CP.CoolPad.CoolPad_PrcGroup_ID
                            Dim iPsPRice_ID As Integer = 0 'PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_PSPrice_ID
                            Dim iSwap_BillCode_ID As Integer = 0 'PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_BillCode_ID
                            Dim strWorkStation As String = "" 'PSS.Data.Buisness.CP.CoolPad.CoolPad_DeviceSwap_WorkStation
                            Dim dPartCharge As Single = 0.0
                            Dim dLaborCharge As Single = 0.0
                            Dim strPartNum As String = ""
                            ...
                            If Me._iCust_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_CUSTOMER_ID Then
                                iLaborLevel = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_LabelLevel
                                iPrcGroup_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_PrcGroup_ID
                                iPsPRice_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_PSPrice_ID
                                iSwap_BillCode_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_Swap_BillCode_ID
                                strWorkStation = PSS.Data.Buisness.CP.CoolPad.CoolPad_DeviceSwap_WorkStation
                            ElseIf Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID Then
                                iLaborLevel = PSS.Data.Buisness.WIKO.WIKO.WIKO_Swap_LabelLevel
                                iPrcGroup_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_PrcGroup_ID
                                iPsPRice_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_Swap_PSPrice_ID
                                iSwap_BillCode_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_Swap_BillCode_ID
                                strWorkStation = PSS.Data.Buisness.WIKO.WIKO.WIKO_DeviceSwap_WorkStation
                            Else
                            ...
                            If MsgBox("Do you want to swap it now?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                                'get labor charge
                                dtTmp = Me._objCoolPad_Swap.getLaborChargeData(iLaborLevel, iPrcGroup_ID)
                                'This query may return multiple rows.  
                                If Not dtTmp.Rows.Count > 0 Then
                                    MessageBox.Show("Can't find labor charge data for the labor level " & iLaborLevel.ToString & ". See IT.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                    Me.txtSeedStockSN.SelectAll() : Me.txtSeedStockSN.Focus() : Exit Sub
                                Else
                                    'The first row is selected to get the labor charge
                                    dLaborCharge = Convert.ToSingle(dtTmp.Rows(0).Item("LaborPrc_RegPrc"))
                                End If
                                'Get part number
                                dtTmp = Me._objCoolPad_Swap.getPartNumberData(iPsPRice_ID)
        ...
        Private Sub CheckSwapDevice(ByVal dt As DataTable, ByVal dtBill As DataTable, ByRef strMsg As String, ByRef bIsNormalRepairedDevice As Boolean)
            ...
            If Me._iCust_ID = PSS.Data.Buisness.CP.CoolPad.CoolPad_CUSTOMER_ID Then 'CoolPad ===============================================================================
                foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.CP.CoolPad.CoolPad_BER_BillCode_ID) 'BER
                If foundRows.Length > 0 Then
                    strMsg = "" : Exit Sub
                Else
                    foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.CP.CoolPad.CoolPad_RUR_BillCode_ID) 'RUR Physical Abuse
                    If foundRows.Length > 0 Then
                    ...
                    Else
                        foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.CP.CoolPad.CoolPad_RUR_BillCode_ID2) 'RUR Liquid Damage
                        If foundRows.Length > 0 Then
                    ...
                    End If
                End If
            ElseIf Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID AndAlso iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCricket_LOC_ID Then 'WIKO Cricket =====
                If iWrty = 1 Then 'WEX, IW
                    foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.WIKO.WIKO.WIKO_BER_BillCode_ID) 'BER
                    If foundRows.Length > 0 Then
                        strMsg = "" : Exit Sub
                    Else
                        foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.WIKO.WIKO.WIKO_RUR_BillCode_ID) 'RUR Physical Abuse
                        If foundRows.Length > 0 Then
                            strMsg = "Device is IW and RUR Physical Abuse. It is not eligible for swap." : Exit Sub
                        Else
                            foundRows = dtBill.Select("BillCode_ID=" & PSS.Data.Buisness.WIKO.WIKO.WIKO_RUR_BillCode_ID2) 'RUR Liquid Damage
                            If foundRows.Length > 0 Then
                ...
                End If
            ElseIf Me._iCust_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID AndAlso (iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_LOC_ID _
                   OrElse iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttFedEx_LOC_ID) Then 'WIKO ATT CTDI and FedEx ======================================================
                If Not dt.Rows(0).IsNull("VendorID") Then strATT_WexPos = Convert.ToString(dt.Rows(0).Item("VendorID"))
                If strATT_WexPos.Trim.ToUpper = PSS.Data.Buisness.WIKO.WIKO.WIKO_FexEx_PosCode.Trim.ToUpper Then strMsg = "Device is POS. We are holding this and it is not eligible for swap now." : Exit Sub
