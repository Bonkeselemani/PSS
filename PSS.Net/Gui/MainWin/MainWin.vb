    ...
    Public Class MainWin
		...
		Public Sub New()
			...
			Try
				...
				Dim bRunTask As Boolean = False : Dim i As Integer = 0
				strCommandLineArgs = Environment.GetCommandLineArgs()

				If strCommandLineArgs.Length > 1 AndAlso strCommandLineArgs(1).Trim.ToLower <> "/appstartversion" Then
					bRunTask = True
					For i = 0 To strCommandLineArgs(1).Length - 1
						If Char.IsDigit(strCommandLineArgs(1).Chars(i)) = False Then bRunTask = False
					Next
					If bRunTask Then
						Gui.MainWin.ModTasks.RunTasks(CInt(strCommandLineArgs(1)))
...
#Else
                ...
                If Updater.Update = True Then End
...
#End If
			...
			Catch ex As Exception
				If InStr(ex.Message, "A connection attempt failed") > 0 Then
					MessageBox.Show("Unable to connect to the database.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
				Else
					MessageBox.Show(ex.ToString, "Gui.MainWin.New()", MessageBoxButtons.OK, MessageBoxIcon.Error)
				End If
				End
			End Try
		End Sub
		...
		Private Function ScheduleTasks(ByVal iInputVal As Integer)
			Try
				End
			Catch ex As Exception
				MessageBox.Show(ex.ToString, "MainWindow.ScheduleTasks", MessageBoxButtons.OK, MessageBoxIcon.Error)
			End Try
		End Function

        Private Sub MainWin_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
