...
#End Region
        ...
        '***************************************************************************
        Public Function GetDataTable(ByVal [string] As String) As DataTable
            ...
            Catch ex As Exception
                If Not IsDBNull(_dt) Then
                    _dt.Dispose()
                End If
                If Not IsNothing(_dt) Then
                    _dt = Nothing
                End If
                Throw New Exception("MotorolaSubcontract_Data.GetDataTable: " & ex.Message.ToString)
            Finally
                _da.Dispose()
                _da = Nothing
                _conn.Close()
        ...
        '***************************************************************************
        Public Function ExecuteNonQueries(ByVal sSQL As String) As Integer
            Dim i As Integer = 0

            Try
                _conn = Connection.GetConnection()
                '_conn.Open()
                Dim _cmd As New MySqlCommand(sSQL, _conn)
                i = _cmd.ExecuteNonQuery()

            Catch ex As Exception
                Throw New Exception("MotorolaSubcontract_Data.ExecuteNonQueries: " & ex.Message.ToString)
            Finally
                _conn.Close()
                _conn.Dispose()
                _conn = Nothing
