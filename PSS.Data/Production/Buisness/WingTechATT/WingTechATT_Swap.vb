...
#End Region
        ...
        Public Function getDeviceData(ByVal iCust_ID As Integer, ByVal iLoc_ID As Integer, ByVal strSN As String, ByVal iBulkORderType_ID As Integer, _
                                      ByVal strWingTechATTCricketOrATT As String) As DataTable
            Dim strSql As String = ""
            Dim strWingTechATTCricketSeedstock As String = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_SeedStockSourceType_Cricket.Trim.Replace("'", "''")
            Dim strWingTechATTAttSeedstock As String = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_SeedStockSourceType_ATT.Trim.Replace("'", "''")
            Dim strWingTechATTFilterOutSeedStock As String = "'" & strWingTechATTCricketSeedstock & "'" & ",'" & strWingTechATTAttSeedstock & "'"
            ...
            Try
                ...
                If iCust_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_CUSTOMER_ID Then
                    If iBulkORderType_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_OrderTypeBulk_ID Then 'None seedstock
                        ' strSql &= " WHERE A.Cust_ID=" & iCust_ID & " AND A.Account NOT IN (" & strWingTechATTFilterOutSeedStock & ")" & Environment.NewLine
                        strSql &= " WHERE A.Cust_ID=" & iCust_ID & " " & Environment.NewLine
                    ElseIf iBulkORderType_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_OrderTypeSeedStock_ID Then 'Seedstock
                        If iLoc_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_AttCricket_LOC_ID _
                           AndAlso strWingTechATTCricketOrATT.Trim.ToUpper = strWingTechATTCricketSeedstock.Trim.ToUpper Then 'Cricket
                            strSql &= " WHERE A.Cust_ID=" & iCust_ID & " AND" & Environment.NewLine
                            'S690C can be used for ATT and Cricked 
                            strSql &= " (CASE WHEN F.Model_id=5251 THEN A.Account IN(" & strWingTechATTFilterOutSeedStock & ") ELSE  A.Account  IN ('" & strWingTechATTCricketSeedstock & "')END)" & Environment.NewLine

                        ElseIf (iLoc_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_AttCTDI_LOC_ID OrElse iLoc_ID = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_AttFedEx_LOC_ID) _
   AndAlso strWingTechATTCricketOrATT.Trim.ToUpper = strWingTechATTAttSeedstock.Trim.ToUpper Then                         'ATT
                            strSql &= " WHERE A.Cust_ID=" & iCust_ID & " AND " & Environment.NewLine
                            'S690C can be used for ATT and Cricked and the model_id of it is 5251
                            strSql &= " (CASE WHEN F.Model_id=5251 THEN A.Account IN(" & strWingTechATTFilterOutSeedStock & ") ELSE  A.Account  IN ('" & strWingTechATTAttSeedstock & "')END)" & Environment.NewLine
                        Else
                            Throw New Exception("Failed to determine WingTechATT seedstock location.")
                        End If
                    Else
                        Throw New Exception("Failed to determine WingTechATT device info.")
                    End If
                Else
                    strSql &= " WHERE A.Cust_ID=" & iCust_ID & " and A.Loc_ID=" & iLoc_ID & " AND A.BulkORderType_ID=" & iBulkORderType_ID & Environment.NewLine
                End If
        ...
        Public Function UpdateSwappedData(ByVal iDevice_ID As Integer, ByVal iOriginal_Device_ID As Integer, ByVal iSwap_BillCode_ID As Integer, ByVal strPartNum As String, _
                                          ...
                                          ByVal dPartCharge As Single, ByVal iLaborLevel As Integer, ByVal strWorkStation As String, ByVal iLoc_id As Integer) As Integer
            Dim strSql As String = ""
            Dim i As Integer = 0
            Dim bRet As Boolean = False
            ...
            Try
                ...
                strSql = "UPDATE production.tDevice SET Device_DateBill='" & strDateTime & "',Loc_id=" & iLoc_id & ", Device_LaborLevel=" & iLaborLevel & ", Device_LaborCharge=" & dLaborCharge & ", Device_PartCharge = " & dPartCharge & " WHERE device_ID =" & iDevice_ID
                i += Me._objDataProc.ExecuteNonQuery(strSql)
                ...
                strSql = "UPDATE production.Extendedwarranty SET Loc_id = " & iLoc_id & " WHERE device_ID = " & iDevice_ID
                i += Me._objDataProc.ExecuteNonQuery(strSql)
