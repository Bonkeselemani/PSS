...
#Region "Report"
        ...
        Public Function CreateInvoiceCricket(ByVal iCust_ID As Integer, ByVal strRptName As String, ByVal dateRec As String, ByVal dateEnd As String, ByVal strLocation As String, ByVal iReportType As Integer, ByVal iOption As Integer, Optional ByVal strImei As String = "") As Integer
            ...
            Else
                strSql &= " WHERE   bulkordertype_id=1 AND C.LOC_ID=" & Locid & "  AND C.pallett_shipdate BETWEEN '" & dateRec & "' AND '" & dateEnd & "' and  pallett_name not like '%2630SDS%'   " & Environment.NewLine
            ...
            Else
                strSql &= " WHERE  C.LOC_ID=" & Locid & "  AND C.pallett_shipdate BETWEEN '" & dateRec & "' AND '" & dateEnd & "'  and  pallett_name not like '%2630SDS%' " & Environment.NewLine
                strSql &= "GROUP BY D.device_id  order by pallett_name )" & Environment.NewLine
            End If
        ...
        Public Function CreateRawDataExcelFileInvoice(ByRef dt1 As DataTable, ByRef dt2 As DataTable, ByVal strFromDt As String, _
            ...
            Try
                ...
                objSheet.Name = " VINSMART INVOICE REPORT" 'Select a Sheet 1 for this
        ...
        Public Function CreateWexATT(ByVal iCust_ID As Integer, ByVal strRptName As String, ByVal dateRec As String, ByVal dateEnd As String, ByVal strLocationId As String, ByVal iStatus As Integer) As Integer
            ...
            Dim strPath As String = "P:\Vinsmart\WEX\"
            Dim strDate As String = dtTime.ToString("yyyyMMddHHmmss")
            Dim dtRow As DataRow
            Dim strSQLTemp As String
            If strLocationId = "ATT" Then
                Locid = (PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_AttCTDI_LOC_ID & "," & PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_AttFedEx_LOC_ID).ToString
            Else
        ...
        Public Function CreateInvoiceATTCTDI(ByVal iCust_ID As Integer, ByVal strRptName As String, ByVal dateRec As String, ByVal dateEnd As String, ByVal strLocation As String, ByVal iReportType As Integer, ByVal iOption As Integer, Optional ByVal strImei As String = "") As Integer
            ...
            Try
                ...
                strSql3 &= "inner join tdevice D ON D.device_id=A.device_id " & Environment.NewLine
                strSql3 &= "inner join tpackingslip E ON E.Pkslip_ID=C.Pkslip_ID " & Environment.NewLine
                strSql3 &= "WHERE  C.Pallett_shipdate BETWEEN '" & dateRec & "' AND '" & dateEnd & "'AND C.loc_id=" & Locid & "  and  pallett_name not like '%2630SDS%'  AND A.ACCOUNT NOT IN ('CRICKET','ATT') " & Environment.NewLine
                ...
                strSql3 &= "inner join tdevice D ON D.device_id=A.Swapped_Device_ID " & Environment.NewLine
                strSql3 &= "inner join tpackingslip E ON E.Pkslip_ID=C.Pkslip_ID " & Environment.NewLine
                strSql3 &= "WHERE  C.Pallett_shipdate BETWEEN '" & dateRec & "' AND '" & dateEnd & "'AND C.loc_id=" & Locid & "  and  pallett_name not like '%2630SDS%' " & Environment.NewLine
                dtSummary = Me._objDataProc.GetDataTable(strSql3)
        ...
        Public Function CreateInventoryReport(ByVal iCust_ID As Integer, ByVal strRptName As String, ByVal dateRec As String, ByVal dateEnd As String, ByVal strLocation As String, ByVal iReportType As Integer, ByVal iOption As Integer, Optional ByVal strImei As String = "") As Integer
            ...
            Try
                ...
                Else
                    strCol = "IF(Device_sn IS NULL ,SerialNo,Device_sn)as SN"
                End If
                If strLocation = "ATT" Then
                    Locid = (PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_LOC_ID & "," & PSS.Data.Buisness.WIKO.WIKO.WIKO_AttFedEx_LOC_ID).ToString
                Else
                    Locid = getLocationId(strLocation)
                End If
                ...
                Select Case iReportType
                    ...
                    Case 3, 7 'Shipped 
                        ...
                        Else
                            strSql &= " WHERE A.sourcefile not like '%seed%' AND A.Cust_ID=" & iCust_ID & " AND  A.LOC_ID=" & Locid & " AND pkslip_createDT BETWEEN '" & dateRec & "' AND '" & dateEnd & "' and  pallett_name not like '%sds%' Group BY E.device_id " & Environment.NewLine
                        End If
                        ...
                        Else
                            strSql &= " WHERE A.Cust_ID=" & iCust_ID & " AND  A.LOC_ID=" & Locid & " AND pkslip_createDT BETWEEN '" & dateRec & "' AND '" & dateEnd & "' and  pallett_name not like '%sds%'Group BY E.device_id " & Environment.NewLine
                        End If
                    Case 4 'Status 
                        strTempQuery = strSql
                        strSql &= " LEFT OUTER JOIN production.tdevice E ON E.device_id=A.device_id " & Environment.NewLine
                        strSql &= " LEFT JOIN tpallett P ON P.Pallett_ID=E.Pallett_ID " & Environment.NewLine
                        If iOption = 1 Then
                            strSql &= " WHERE device_SN IN ( " & strImei & " ) and A.sourcefile not like '%REF%'  Group BY E.device_id " & Environment.NewLine
                        Else
                            strSql &= " WHERE  A.cust_ID=" & iCust_ID & " AND  A.LOC_ID IN (" & Locid & ") AND SOURCEFILE NOT LIKE '%Seed%'  AND( pallett_name NOT LIKE '2630%' OR  pallett_name IS NULL)   " & Environment.NewLine
                        End If
                        strSql &= " UNION " & Environment.NewLine
                        strSql &= strTempQuery
                        strSql &= " INNER JOIN tdevice E ON E.device_id=swapped_device_id " & Environment.NewLine
                        strSql &= " INNER JOIN tpallett P ON P.Pallett_ID=E.Pallett_ID" & Environment.NewLine
                        If iOption = 1 Then
                            strSql &= " WHERE device_SN IN ( " & strImei & " ) " & Environment.NewLine
                        Else
                            strSql &= " WHERE  A.cust_ID=" & iCust_ID & " AND  A.LOC_ID IN (" & Locid & ")   AND ( pallett_name NOT LIKE '%2630%' OR  pallett_name IS NULL)    " & Environment.NewLine
                        End If
                    Case Else
                        Return 0
                End Select
        ...
        '        strSql &= " WHERE A.Cust_ID=2630 AND E.LOC_ID=" & Locid & "  AND E.Device_DateShip BETWEEN '" & dateRec & "' AND '" & dateEnd & "';" & Environment.NewLine
        '        dtSummary = Me._objDataProc.GetDataTable(strSql)
        '        dtSummary.TableName = strRptName
        '        ds.Tables.Add(dtSummary)
        '        objExcelRpt = New ExcelReports(False)
        ...
        Public Sub CreateRawDataExcelFile(ByRef dt1 As DataTable, _
            ...
            Try
                ...
                With objExcel.Selection
                    ...
                    .Font.ColorIndex = 3        'Red
                End With
                objExcel.Application.Cells(1, 1).Value = "Vinsmart Pretest Raw Data Report"
                '*************************************************
        ...
        Public Function CreateRawDataExcelFileInvoiceATT(ByRef dt1 As DataTable, ByVal strFromDt As String, _
            ...
            Try
                ...
                With objExcel.Selection
                    ...
                    .HorizontalAlignment = -4108
                End With
                objExcel.Application.Cells(1, 1).Value = String.Concat("Vinsmart ", strLocation.ToUpper, " INVOICE REPORT")
                '*************************************************
