...
#End Region
        ...
        Public Function GetAccount(ByVal strImei As String, ByVal iCust_Id As Integer) As Integer
            ...
            Try
                ...
                strSql &= "INNER JOIN tdevice B ON A.device_id=B.Device_id" & Environment.NewLine
                strSql &= " where A.SerialNo = '" & strImei & "' AND A.cust_id= " & iCust_Id & "" & Environment.NewLine
                strSql &= " and B.Device_DateShip is null and A.account in ('ATT','Cricket')" & Environment.NewLine
                dt = Me._objDataProc.GetDataTable(strSql)
        ...
        Public Function GetAccount_DOA_POS(ByVal strImei As String, ByVal iCust_Id As Integer) As DataTable
            Dim dtrow As DataRow
            Dim strSql As String = ""
            Dim orderType As Integer
            Dim dt As DataTable
        ...
        Public Function getPallet_MaxQty(ByVal pallett_id As Integer, ByVal iCust_id As Integer) As Integer
            Dim dt As DataTable
            Dim row As DataRow
            Dim strSql As String = ""
            Try
                strSql = "SELECT pallett_maxQty  FROM tpallett WHERE cust_ID =" & iCust_id & " AND Pallett_id = " & pallett_id & ""
                dt = Me._objDataProc.GetDataTable(strSql)

                Return dt.Rows(0)("pallett_maxQty")
            Catch ex As Exception
                Throw ex
            End Try
        End Function

        Public Function GetLocID(ByVal strImeiDeviceSN As String) As Integer
            'NO NEED THIS----------------------------------!
            Dim dt As DataTable
            Dim strSQL As String = String.Empty
            Dim iLocID As Int32
            Try
                strSQL = "SELECT A.Device_ID, A.Device_SN, A.Loc_ID " & Environment.NewLine
                ...
                strSQL = strSQL & "WHERE A.Device_SN = '" & strImeiDeviceSN & "' AND A.Device_DateShip IS NULL "

                dt = Me._objDataProc.GetDataTable(strSQL)

                If dt.Rows.Count > 0 Then
                    Dim dr As DataRow = dt.Rows(0)
                    iLocID = Convert.ToInt32(dr("Loc_ID"))
                    Return iLocID
                End If
        ...
        Public Function GettDeviceBill(ByVal Imei As String) As DataTable
            ...
            Dim dt As DataTable
            Try
                strSql &= "SELECT  BillCode_id,A.device_id " & Environment.NewLine
                strSql &= " FROM tdevice A " & Environment.NewLine
                strSql &= " INNER JOIN tDeviceBill B ON  A.device_id = B.device_id" & Environment.NewLine
                strSql &= " where A.device_SN = '" & Imei & "' AND device_dateship  IS NULL;" & Environment.NewLine
                dt = Me._objDataProc.GetDataTable(strSql)
                Return dt
            Catch ex As Exception
                Throw ex
        ...
        Public Function GetWiKoLocationsbyID(ByVal iCust_ID As Integer, ByVal strIMEI As String) As DataTable
            Dim strSql As String = ""
            Dim dt As DataTable
        ...
        Public Function GetWiKoModels(ByVal iCust_ID As Integer, ByVal booAddSelectRow As Boolean) As DataTable
            ...
            Dim dt As DataTable
            Try
                strSql = "Select  Model_ID, Model_Desc from tmodel WHERE cust_ids=" & iCust_ID & Environment.NewLine
                dt = Me._objDataProc.GetDataTable(strSql)
                If booAddSelectRow Then dt.LoadDataRow(New Object() {0, "--Select--"}, True)
                Return dt
            Catch ex As Exception
        ...
        Public Function CreateBoxID(ByVal iModelID As Integer, _
                                    ...
                                    ByVal strSW_Version As String, ByVal iMaxQty As Integer, Optional ByVal isDOA As Boolean = False) As Integer
            Dim strSql As String = ""
            ...
            Try
                If iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCricket_LOC_ID Then
                    ...
                    '******************************
                    If isDOA Then
                        strAccount = PSS.Data.Buisness.WIKO.WIKO.WIKO_Cricket_OEMCustomer_DOA_AccountCode
                    End If
                ...
                Else
                    ...
                    If isDOA Then
                        strAccount = PSS.Data.Buisness.WIKO.WIKO.WIKO_FexEx_PosCode
                    End If
                    Dim strPalletnameprefix As String = String.Empty

                    Dim strPalletPostFix As String = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_Box_Postfix
                    Dim strDatePallet As String = Generic.GetMySqlDateTime("%d%m%Y")
                    strPalletnameprefix = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_Box_Prefix & strPalletPrefix & strDatePallet & "N"
                    Dim iPalletNameLen As Integer = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_BoxName_Len
                ...
                If Not iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCricket_LOC_ID Then
                    strTempLoc_id = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCTDI_LOC_ID
                Else
                    strTempLoc_id = PSS.Data.Buisness.WIKO.WIKO.WIKO_AttCricket_LOC_ID
                End If
                '******************************
                'Create pallet
                ''******************************
                ...
                End If
                strSql &= ", " & strTempLoc_id & ", " & iMaxQty & ");" & Environment.NewLine
        ...
        Public Function GetWiKoOpenPallets(ByVal iCust_ID As Integer, ByVal iLoc_ID As String) As DataTable
            ...
            Try
                strSql = "SELECT Pallett_ID, tpallett.Model_ID, tpallett.Loc_ID, Pallet_ShipType, if(Pallett_QTY is null,0,Pallett_QTY) as Pallett_QTY,Pallett_Name as 'Box Name',IF (tpallett.Loc_ID=4484,tlocation.Loc_Name,'ATT ') as Location, Model_Desc as Model,Pallett_MaxQty as MaxQty,Account" & Environment.NewLine
        ...
        Public Function GetDeviceSIMcard(ByVal iDevice_ID As Integer) As DataTable
            ...
            Try
                ...
                strSql &= " WHERE B.Cust_ID=" & PSS.Data.Buisness.WIKO.WIKO.WIKO_CUSTOMER_ID & " AND B.LOC_ID=" & PSS.Data.Buisness.WIKO.WIKO.WIKO_Special_LOC_ID & "  and Device_ID =" & iDevice_ID & " ;" & Environment.NewLine
                Return Me._objDataProc.GetDataTable(strSql)
            Catch ex As Exception
                Throw ex
            End Try
        ...
        Public Function PrintBoxLabel(ByVal iPalletID As Integer) As Integer
            Dim strReportName As String = "WiKo_Pallet_Cricket_Label_20.rpt" ' "WiKo_Pallet_Label_" '"WiKo_Pallet_Label_20.rpt"
            Dim strReportName1 As String = "WiKo_Pallet_Cricket_Label_50.rpt"
            Dim strSql As String
        ...
        Public Function Print_AttCTDI_BoxLabel(ByVal iPalletID As Integer) As Integer
            Dim strReportName As String = "WiKo_Pallet_AttCTDI_Label_20.rpt"
            Dim strReportName1 As String = "WiKo_Pallet_AttCTDI_Label_50_2.rpt"
            Dim strSql As String
            ...
            Try
                ...
                strSql &= "  ,if(A.Pallet_ShipType=1,'N/A',A.SW_Version)as SW_Version,if(A.Pallet_ShipType=1,'N/A',ClaimNo) as 'OrderNo',if(A.Pallet_ShipType=0, Concat('REFURBISHED ',C.Model_Desc),if(A.Pallet_ShipType=2, Concat('RUR ',C.Model_Desc),if(A.Pallet_ShipType=1, Concat('BER ',C.Model_Desc),''))) as 'PalletType'" & Environment.NewLine
                ...
                strSql &= " INNER JOIN tmodel C ON B.Model_ID=C.Model_ID" & Environment.NewLine
                strSql &= " WHERE A.Pallett_ID =" & iPalletID & Environment.NewLine
                strSql &= "AND if(A.Pallet_ShipType<>1, E.Account not in ('ATT','CRICKET' ),E.Item_SKU IS NOT NULL)" & Environment.NewLine
