...
Namespace Buisness.WIKO
    Public Class WIKO_SpecialProject
...
#End Region
        ...
        Public Function GetWiKoShipBoxTypes(ByVal booAddSelectRow As Boolean) As DataTable
            Dim strSql As String = ""
            Dim dt As DataTable
            Dim drNewRow As DataRow

            Try
                strSql = "SELECT 0 as 'ShipTypeID', 'P' as 'ShipTypeSDesc', 'PASS' as 'ShipTypeLDesc' " & Environment.NewLine
                strSql &= "UNION ALL SELECT 1 as 'F', 'FAIL' as 'ShipTypeSDesc', 'FAIL' as 'ShipTypeLDesc'"
        ...
        Public Function UpdateWikoPalletLocation(ByVal iLoc_ID As Integer, ByVal iPallett_ID As Integer) As Integer
            Dim strSql As String = ""
            Dim dt As DataTable
            Try
        ...
        Public Function CreateBoxID(ByVal iModelID As Integer, _
            ...
            Dim dt As DataTable
            Try
                If iLoc_ID = PSS.Data.Buisness.WIKO.WIKO.WIKO_Special_LOC_ID Then
                    '******************************
                    'construct pallet name
                    '******************************
                    strDate = Generic.GetMySqlDateTime("%y%m%d")

                    If Len(sCust_PO) <> 4 Then   ' For SP1 Project no need to add POnumber in BoxName
                        strPalletPrefix = "PL102"
                    Else
                        strPalletPrefix = "PL102"
                    End If
                    ...
                    If iMaxNum > 999999 Then Throw New Exception("Max pallets (per location per day) hit the 999999 limit." & Environment.NewLine)
        ...
        Private Function DefinePalletName(ByVal strPalletPrefix As String, ByRef iMaxNum As Integer, ByVal iCust_ID As Integer) As String
            ...
            Try
                'up to 999
                strSQL = "SELECT  if(max(right(Trim(Pallett_Name), 6) ) is null,1,max(right(Trim(Pallett_Name),6) ) + 1) as  Pallett_Num" & Environment.NewLine
                strSQL &= " , if(max(right(Trim(Pallett_Name), 6) ) is null,CONCAT('" & strPalletPrefix & "', lPad(1,6,'0')) ,CONCAT( '" & strPalletPrefix & "', lPad(max(right(Trim(Pallett_Name),6) ) + 1, 6,'0'))) as  Pallett_Name" & Environment.NewLine
        ...
        Public Function GetWiKoOpenPallets(ByVal iCust_ID As Integer, ByVal iLoc_ID As String) As DataTable
            Dim strSql As String
            ...
            Try
                ...
                strSql &= " tpallett.Account as Project, CASE  " & Environment.NewLine
                strSql &= " WHEN tpallett.PalletType_ID = 0 then 'Pass' ELSE 'Fail' END AS BoxType  " & Environment.NewLine
        ...
        Public Function GetWiKoPallettData(ByVal strPallettName As String, ByVal iCust_ID As Integer) As DataTable
            Dim dt As DataTable
            Dim row As DataRow
            Dim strSql As String = ""
        ...
        Public Function GetDeviceAccount(ByVal iDevice_ID As Integer) As DataTable
            Dim strSql As String = ""
            Try
                strSql = "SELECT A.account FROM  extendedwarranty A INNER JOIN tdevice B ON A.Device_ID=B.device_id WHERE B.Device_ID =" & iDevice_ID & "  and device_DateShip is null;" & Environment.NewLine
                Return Me._objDataProc.GetDataTable(strSql)
            Catch ex As Exception
                Throw ex
            End Try
        ...
        Public Function CloseWiKoPallet(ByVal iCust_ID As Integer, _
        ...
        Public Function GetPallet_SN(ByVal iPalletID As Integer) As DataTable
            ...
            Dim dt As DataTable
            Try
                strSql = "SELECT Cust_Po as PO,OEM_RA as Partnumber,cntry_name,D.Item_Desc, A.Pallett_Name as Pallet,C.Model_Desc as Model,A.Pallett_Qty as Qty,B.Device_SN as SN,B.Device_ID" & Environment.NewLine
        ...
        Public Function ReopenWiKoBoxByResetting(ByVal iPalletID As Integer, ByVal strStation As String) As Integer
            Dim strSql As String = ""
            Try
        ...
        Public Function getWiKoPalletDataForSP_MP(ByVal iPalletID As Integer) As DataTable
            'Data fro Special prject and requires master pallet (shipping pallet)
            Dim strSql As String = ""
            Try
        ...
        Public Function getWingTechAttGenericProcessPalletDataForSP_MP(ByVal iPalletID As Integer, _
                                                                       ByVal strAccount As String, _
                                                                       ByVal strCountry As String) As DataTable
            'Data fro WingTech ATT Special prject (Generic Process) and requires master pallet (shipping pallet)
            Dim strSql As String = ""
            Try
                strSql = "SELECT '" & strAccount & "' AS Account,B.Pallett_Name,'" & strCountry & "' AS Cntry_Name,'' AS OEM_RA,B.Cust_PO,A.Model_ID,A.loc_ID,B.Pallett_ID,B.Pallett_Qty,Count(*) as Qty, if(B.Pallett_Qty=count(*),0,1) AS QtyOk_Yes0No1" & Environment.NewLine
                strSql &= " FROM tdevice A" & Environment.NewLine
                strSql &= " Inner join tpallett B ON A.Pallett_ID=B.Pallett_ID" & Environment.NewLine
                strSql &= " WHERE B.Pallett_ID=" & iPalletID & Environment.NewLine
                strSql &= " GROUP BY  B.Pallett_Name, B.Cust_PO,A.Model_ID,A.loc_ID,B.Pallett_ID;" & Environment.NewLine
        ...
        Public Function getSP_MP_PalletQty(ByVal strPallett_IDs As String, ByRef iDevQtyPerBox As Integer, ByRef bIsSameDeviceQtyPerBox As Boolean) As Integer
            Dim strSql As String = ""
            Dim dt1, dt2 As DataTable
            Dim iQty As Integer = 0
            Dim iTmpQtyOld As Integer = 0, iTmpQtyNew As Integer = 0
            Dim i As Integer = 0
            Dim row As DataRow

            Try

                bIsSameDeviceQtyPerBox = True
                ...
                If dt1.Rows.Count > 0 Then
                    For Each row In dt1.Rows
                        If i = 0 Then
                            iTmpQtyOld = Convert.ToInt32(row("Pallett_Qty"))
                        Else
                            iTmpQtyNew = Convert.ToInt32(row("Pallett_Qty"))
                            If Not iTmpQtyOld = iTmpQtyNew Then bIsSameDeviceQtyPerBox = False : Exit For
                        End If
                        i += 1
                    Next
                Else 'No data
                    iQty = 0 : iDevQtyPerBox = 0 : Return iQty
                End If
                ...
                If bIsSameDeviceQtyPerBox Then 'Device qty per box is the same
                    If dt2.Rows.Count > 0 Then iQty = dt2.Rows(0).Item("Qty") : iDevQtyPerBox = dt2.Rows(0).Item("avgQty") 'should be 1 record
                Else 'Device qty per box is NOT the same
                    If dt2.Rows.Count > 0 Then iQty = dt2.Rows(0).Item("Qty") : iDevQtyPerBox = -1
                End If
        ...
        Public Function getSP_MP_DeviceQty(ByVal strPallett_IDs As String) As Integer
            Dim strSql As String = ""
            Dim dt As DataTable
            Dim iQty As Integer = 0
        ...
        Public Function SaveMasterPallet(ByVal strMPallet_Name As String, ByVal strMLoadNo As String, ByVal strPO As String, ByVal strMftPart As String, ByVal strCustPart As String, _
                                         ...
                                         ByVal iPkSlip_ID As Integer, ByVal iCust_ID As Integer, ByVal iLoc_ID As Integer, ByVal bIsSameDeviceQtyPerBox As Boolean) As Integer

            Dim strSql As String = ""
            Dim iIsSameQtyPerBox As Integer = 1

            Try
                If Not bIsSameDeviceQtyPerBox Then iIsSameQtyPerBox = 0

                strSql = "INSERT INTO  production.tpackingslippallet (MPallet_Name, MLoadNo, PO, Desc1, Desc2, Desc3, Qty1, Qty2, Qty3, pkslip_ID, Cust_ID, Loc_ID,IsSameQtyPerBox)"
                strSql &= "VALUES ('" & strMPallet_Name & "','" & strMLoadNo & "','" & strPO & "','" & strMftPart & "','" & strCustPart & "','" & _
                                   strCountry & "'," & iDevQtyPerBox & "," & iBoxQtyMasterPallet & "," & iToalDevQty & "," & iPkSlip_ID & "," & iCust_ID & "," & iLoc_ID & "," & iIsSameQtyPerBox & ");"
                Return Me._objDataProc.ExecuteNonQuery(strSql)
        ...
        Public Function getWIKO_SpTellerManifest_MostRecent_pkSlipID() As Integer
            Dim strSql As String = ""
            Dim dt As DataTable
        ...
        Public Function getSP_MP_ManifestMasterPalletData(ByVal ipkSlip_ID As Integer) As DataTable
            Dim strSql As String = ""
        ...
        Public Function PrintManifestMasterPalletLabel(ByVal strMasterPalletName As String, ByVal strPO As String, ByVal strLoadNo As String, _
                                                       ...
                                                       ByVal iLabelCopyNum As Integer, ByVal bIsSameDeviceQtyPerBox As Boolean, Optional ByVal Cust_id As Integer = 0, Optional ByVal CountryOfOrigine As String = "") As Integer
            Dim strReportName As String
            Dim strReportName2 As String
            If Cust_id = PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_CUSTOMER_ID Then
                strReportName = "Vinsmart Master pallett.rpt"
            ElseIf Cust_id = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_CUSTOMER_ID Then
                strReportName = "WingTechATT_SP_Master_Pallet_Label.rpt"
            Else
                strReportName = "WiKo_Pallet_SP_Master_Pallet_Label.rpt"   'for all Wiko Special Project Teller Data Manifest Master Pallet label
                strReportName2 = "WiKo_Pallet_SP_Master_Pallet_Label_Two.rpt"
            End If
            Dim strSql As String = ""
            Dim strSql2 As String = ""
            Dim dtLabel As DataTable, dtModel As DataTable
            ...
            Dim strCountryOfOrigine As String = String.Empty
            Dim strCountryOfOrigine_Code As String = String.Empty
            ...
            Try
                ...
                If Cust_id = PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_CUSTOMER_ID Then
                    strS = CountryOfOrigine.Trim : strCountryOfOrigine = ReplaceChar(strS) : strCountryOfOrigine_Code = FontEncoder.Code128b(strS)
                End If
                ...
                If Cust_id = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_CUSTOMER_ID Then
                    strSql2 = "SELECT B.Model_ID,C.Model_Desc,C.ShippedModel_Desc" & Environment.NewLine
                    strSql2 &= "  FROM production.tpackingslippallet A" & Environment.NewLine
                    strSql2 &= "  INNER JOIN production.tpallett B ON A.pkslip_id=B.pkslip_id" & Environment.NewLine
                    strSql2 &= "  INNER JOIN production.tmodel C ON B.Model_ID=C.Model_ID" & Environment.NewLine
                    strSql2 &= "  WHERE A.Cust_ID=" & Cust_id & " AND A.Mpallet_Name ='" & strMasterPalletName.Trim & "' Limit 1;" & Environment.NewLine
                    dtModel = Me._objDataProc.GetDataTable(strSql2)
                    If dtModel.Rows.Count > 0 AndAlso Not dtModel.Rows(0).IsNull("ShippedModel_Desc") Then
                        strModel = dtModel.Rows(0).Item("ShippedModel_Desc")
                        strModel_Code = ReplaceChar(FontEncoder.Code128b(strModel))
                    End If

                    strSql &= ", '" & strModel & "' AS 'Model','" & strModel_Code & "' AS 'ModelCode'"
                End If
                ...
                If Cust_id = PSS.Data.Buisness.Vinsmart.Vinsmart.Vinsmart_CUSTOMER_ID Then
                    strSql &= ",'" & strCountryOfOrigine & "' AS 'CountryOfOrigine','" & strCountryOfOrigine_Code & "' AS 'CountryOfOrigineCode' " & Environment.NewLine
                End If
                dtLabel = Me._objDataProc.GetDataTable(strSql)
                ...
                If dtLabel.Rows.Count > 0 Then
                    ...
                    ElseIf Cust_id = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_CUSTOMER_ID Then
                        PSS.Data.Buisness.TracFone.clsMisc.PrintCrystalReportLabel(dtLabel, strReportName, iLabelCopyNum)
                    Else
                        If bIsSameDeviceQtyPerBox Then
                            PSS.Data.Buisness.TracFone.clsMisc.PrintCrystalReportLabel(dtLabel, strReportName, iLabelCopyNum)
                        Else
                            PSS.Data.Buisness.TracFone.clsMisc.PrintCrystalReportLabel(dtLabel, strReportName2, iLabelCopyNum)
                        End If
                    End If
                    Return dtLabel.Rows.Count
                Else
        ...
        'Old one
        'Public Function getWIKO_SpTellerManifestMasterPalletDetailedData(ByVal strMPalletNames As String, ByVal iCartonWeight As Integer) As DataTable
        '    Dim strSql As String = ""
        ...
        Public Function getWIKO_SpTellerManifestMasterPalletAccount(ByVal strMPalletNames As String) As String
            Dim strSql As String = ""
            Dim dt As DataTable
        ...
        Public Function getWIKO_SpTellerManifestMasterPalletDetailedData(ByVal strMPalletNames As String, _
                    ByVal iCartonWeight As Integer, ByVal strAccount As String) As DataTable
            Dim strSql As String = ""
        ...
        Public Function getGenericProcess_WingTechAttSPManifestMasterPalletDetailedData(ByVal strMPalletNames As String, _
                                                                                        ByVal iCartonWeight As Integer) As DataTable
            Dim strSql As String = ""
            Dim dt As DataTable
            Dim strRes As String = ""
            Dim iCust_ID As Integer = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_CUSTOMER_ID
            Dim iSP_Loc_ID As Integer = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_Special_LOC_ID
            'Dim strAccount As String = PSS.Data.Buisness.WingTechATT.WingTechATT.WingTechATT_SpecialProj_Type1.Trim
            ...
            Try
                ...
                strSql &= " INNER JOIN production.tpallett B ON A.pkslip_id=B.pkslip_id" & Environment.NewLine
                strSql &= " INNER JOIN production.tdevice C ON B.pallett_id=C.pallett_id" & Environment.NewLine
                strSql &= " WHERE A.Cust_ID=" & iCust_ID & " AND A.loc_ID=" & iSP_Loc_ID & " AND A.MPallet_Name IN (" & strMPalletNames & ");" & Environment.NewLine
        ...
        Public Function GetCountryAndAccount(ByVal strSN As String, ByVal iCust_ID As Integer, ByVal strLoc_ID As String) As DataTable
            ...
            Try
                strSN = strSN.Replace("'", "''")
                strSql = "select serialNo as Device_SN,cntry_name,loc_ID, extendedwarranty.Account from extendedwarranty  where serialNo='" & strSN & "' AND Cust_ID = " & iCust_ID & " AND Loc_ID= '" & strLoc_ID & "'"
                Return Me._objDataProc.GetDataTable(strSql)
            Catch ex As Exception
                Throw ex
            End Try
        ...
        ' ************* Charles Hummer (26-Jun-2021) Changed query to get data from the database *************
        Public Function GetWiKoProjectTypes(ByVal iCust_ID As Integer, ByVal iLoc_ID As Integer, ByVal booAddSelectRow As Boolean) As DataTable
            Dim strSql As String = ""
            Dim dt As New DataTable()

            Try
                'strSql = "SELECT '6K202Project' as PrjType_Desc, '6K202Project' as PrjType_Name " & Environment.NewLine
                'strSql &= "UNION ALL SELECT 'U318Project' as PrjType_Desc, 'U318Project' as PrjType_Name " & Environment.NewLine
                'strSql &= "UNION ALL SELECT 'Teller' as PrjType_Desc, 'Teller' as PrjType_Name " & Environment.NewLine
                'strSql &= "UNION ALL  SELECT 'SP1' as PrjType_Desc, 'SP1' as PrjType_Name "
        ...
        Public Function GetWikoAccounts(ByVal iCustID As Integer, ByVal iLocID As Integer) As DataTable
            'Get the accounts (projects) used for Wiko
            Dim strSql As String = ""
            Dim dt As New DataTable()
