...
#Region "CONSTRUCTORS"
		...
		Public Sub New(ByVal device_id As Integer, ByVal pt_id As Integer, ByVal ws_na As String, _
		ByVal wssl_na As String, ByVal user_na As String, ByVal cmp_na As String, ByVal prc_na As String)
...
#Region "PROPERTIES"
		...
		Public Property prc_na() As String
			...
			End Set
		End Property
		Public ReadOnly Property crt_dt() As Date
			Get
				Return _crt_dt
			End Get
		End Property
...
#End Region
	...
	Public Class tdevice_activity_statsCollection
#Region "DECLARATIONS"
...
#Region "CONSTRUCTORS"
		...
		Public Sub New(ByVal product_type_id As Integer, ByVal crt_dt As String)
			_objDataProc = New DBQuery.DataProc(ConfigFile.GetConnectionInfo)
			GetData(product_type_id, crt_dt)
		End Sub
...
#Region "METHODS"
		...
		Protected Sub GetData(ByVal product_type_id As Integer, ByVal crt_dt As String)
			Dim _sql As String = GetSelectStatement(product_type_id, crt_dt)
			_dt = _objDataProc.GetDataTable(_sql)
		End Sub

		Protected Function GetSelectStatement(ByVal product_type_id As Integer, ByVal crt_dt As String) As String
			Dim _sql As String
			_sql = "SELECT "
			_sql += "user_na, "
			_sql += "prc_na, "
			_sql += "ws_na, "
			_sql += "count(device_id) as cnt "
			_sql += "FROM production.tdevice_workstation_journal "
			_sql += "WHERE  "
			_sql += "pt_id = " & product_type_id.ToString() & " AND "
			_sql += "crt_dt between '" & crt_dt & " 00:00:00' AND  '" & crt_dt & " 23:59:59' "
			_sql += "GROUP BY "
			_sql += "user_na, "
			_sql += "prc_na, "
			_sql += "ws_na "
			_sql += "ORDER BY "
			_sql += "user_na, ws_na, prc_na; "
			Return _sql
		End Function

		Protected Function GetDetailsSelectStatement(ByVal product_type_id As Integer, ByVal crt_dt As String, ByVal user_na As String, ByVal prc_na As String, ByVal ws_na As String)
			Dim _sql As String
			_sql = "SELECT "
			_sql += "j.dwsj_id, "
			_sql += "j.device_id, "
			...
			_sql += "WHERE  "
			_sql += "pt_id = " & product_type_id.ToString() & " AND "
			_sql += "user_na = " & user_na & " AND "
			_sql += "prc_na = " & prc_na & " AND "
			_sql += "ws_na = " & ws_na & " AND "
			_sql += "crt_dt between '" & crt_dt & " 00:00:00' AND  '" & crt_dt & " 23:59:59' "
			_sql += "ORDER BY "
			_sql += " "
			Return _sql
...
#End Region
	...
	Public Class tdevice_activity_stats_detailCollection
#Region "DECLARATIONS"
...
#Region "CONSTRUCTORS"
		...
		Public Sub New(ByVal product_type_id As Integer, ByVal crt_dt As String, ByVal user_na As String, ByVal prc_na As String, ByVal ws_na As String)
			_objDataProc = New DBQuery.DataProc(ConfigFile.GetConnectionInfo)
			GetData(product_type_id, crt_dt, user_na, prc_na, ws_na)
		End Sub
...
#Region "METHODS"
		...
		Protected Sub GetData(ByVal product_type_id As Integer, ByVal crt_dt As String, ByVal user_na As String, ByVal prc_na As String, ByVal ws_na As String)
			Dim _sql As String = GetDetailsSelectStatement(product_type_id, crt_dt, user_na, prc_na, ws_na)
			_dt = _objDataProc.GetDataTable(_sql)
		End Sub
		...
		Protected Function GetDetailsSelectStatement(ByVal product_type_id As Integer, ByVal crt_dt As String, ByVal user_na As String, ByVal prc_na As String, ByVal ws_na As String)
			...
			_sql += "j.prc_na, "
			_sql += "j.crt_dt "
			_sql += "FROM production.tdevice_workstation_journal j "
			_sql += "LEFT JOIN production.tdevice d ON j.device_id = d.device_id "
			_sql += "LEFT JOIN production.lproduct p ON j.pt_id = p.prod_id "
			_sql += "LEFT JOIN production.tmodel m ON d.model_id = m.model_id "
			_sql += "WHERE  "
			_sql += "j.user_na = '" & user_na & "' AND "
			_sql += "j.pt_id = " & product_type_id.ToString() & " AND "
			_sql += "j.ws_na = '" & ws_na & "' AND "
			_sql += "j.prc_na = '" & prc_na & "' AND "
			_sql += "j.crt_dt between '" & crt_dt & " 00:00:00' AND  '" & crt_dt & " 23:59:59' "
			_sql += "ORDER BY "
			_sql += "j.crt_dt "
			Return _sql
